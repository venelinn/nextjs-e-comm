...
import { PrimaryCard, type PrimaryCardProps } from "@/components/Cards";
...
export default async function ProductDetailPage({ params }: { params: Promise<{ lang: string; id: string }> }) {
  ...
  const product: PrimaryCardProps = {
    ...
    // PrimaryCard requires 'heading', so we map 'title' to 'heading' on the card props:
    heading: Object(rawProduct.heading),
  };

  return <PrimaryCard {...product} />;
}
